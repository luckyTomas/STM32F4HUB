...
USB_HID_CLASS, USBH_HID_InterfaceInit, USBH_HID_InterfaceDeInit, USBH_HID_ClassRequest, USBH_HID_Process, USBH_HID_SOFProcess, USBH_HID_Parameter, };
static HID_HandleTypeDef hhids[3];
...
static USBH_StatusTypeDef create_endpoints(USBH_HandleTypeDef *phost, HID_HandleTypeDef *HID_Handle, USBH_InterfaceDescTypeDef *pif) {
    ...
    for (num = 0; num < max_ep; num++) {
        if (pif->Ep_Desc[num].bEndpointAddress & 0x80) {
            ...
            if (HID_Handle->poll[0] < HID_MIN_POLL) HID_Handle->poll[0] = HID_MIN_POLL;

            /* Open pipe for IN endpoint */
            USBH_OpenPipe(phost, HID_Handle->InPipe, HID_Handle->InEp, phost->device.address, phost->device.speed, USB_EP_TYPE_INTR, HID_Handle->length[0]);
            ...
            if (HID_Handle->poll[1] < HID_MIN_POLL) HID_Handle->poll[1] = HID_MIN_POLL;

            /* Open pipe for OUT endpoint */
            USBH_OpenPipe(phost, HID_Handle->OutPipe, HID_Handle->OutEp, phost->device.address, phost->device.speed, USB_EP_TYPE_INTR, HID_Handle->length[1]);

            USBH_LL_SetToggle(phost, HID_Handle->OutPipe, 0);

            USBH_UsrLog("OUT size=%d, num %d, addr 0x%02X, pool %d", HID_Handle->length[1], HID_Handle->OutPipe, HID_Handle->ep_addr[1], HID_Handle->poll[1]);
        }
    }
...
static HID_HandleTypeDef* create_handle(USBH_HandleTypeDef *phost) {
    if (phost->interfaces < sizeof(hhids) / sizeof(hhids[0])) {
        phost->USBH_ClassTypeDef_pData[phost->interfaces] = &hhids[phost->interfaces];
    }
...
static USBH_StatusTypeDef USBH_HID_InterfaceInit(USBH_HandleTypeDef *phost) {
    ...
    case 0: {
        ...
        if (phost->device.CfgDesc.bNumInterfaces > USBH_MAX_NUM_INTERFACES) num_itf = USBH_MAX_NUM_INTERFACES;
...
//Usage (Mouse) 09 02
    ...
    case 4:
        ...
        if (HID_Handle != NULL) create_endpoints(phost, HID_Handle, pif);

        state = 1;

        if (++if_ix >= num_itf) state = 5;
...
USBH_StatusTypeDef USBH_HID_InterfaceDeInit(USBH_HandleTypeDef *phost) {
    ...
    for (; idx < phost->interfaces; ++idx) {
        ...
        if (phost->USBH_ClassTypeDef_pData[idx] != NULL){
            memset(&hhids[idx],0,sizeof(hhids[0]));
        }
...
static USBH_StatusTypeDef USBH_HID_ClassRequest(USBH_HandleTypeDef *phost) {
    ...
    case CTRL_STATE_HID_REQ_SET_PROTOCOL:
        ...
        if (classReqStatus == USBH_OK || classReqStatus == USBH_NOT_SUPPORTED) {
            ...
            if (phost->pUser != NULL) phost->pUser(phost, HOST_USER_CLASS_ACTIVE);
...
static void USBH_HID_IncrementInterface(USBH_HandleTypeDef *phost) {
    ...
    if (phost->device.current_interface >= phost->interfaces) phost->device.current_interface = 0;
}
...
static USBH_StatusTypeDef USBH_HID_Process(USBH_HandleTypeDef *phost) {
    ...
    case HID_INIT:
        if (HID_Handle->Init(phost) == USBH_OK) HID_Handle->state = HID_SYNC;
        break;
...
static USBH_StatusTypeDef hid_request(USBH_HandleTypeDef *phost, uint8_t request, uint8_t dataDirection, uint16_t value, uint16_t index, uint8_t *buffer,
        uint16_t size) {
 ...
 * @retval USBH Status
 */
USBH_StatusTypeDef USBH_HID_SetReport(USBH_HandleTypeDef *phost, uint8_t reportType, uint8_t reportId, uint8_t *reportBuff, uint16_t reportLen) {
 ...
 * @retval USBH Status
 */
USBH_StatusTypeDef USBH_HID_GetReport(USBH_HandleTypeDef *phost, uint8_t reportType, uint8_t reportId, uint8_t *reportBuff, uint16_t reportLen) {
...
uint8_t USBH_HID_GetPollInterval(USBH_HandleTypeDef *phost) {
    ...
    if ((phost->gState == HOST_CLASS_REQUEST) || (phost->gState == HOST_INPUT) || (phost->gState == HOST_SET_CONFIGURATION)
